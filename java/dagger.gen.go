...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "Java":
		...
		case "WithMaven":
			...
			return (*Java).WithMaven(&parent, version), nil
		case "WithJdk":
			var err error
			var parent Java
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*Java).WithJdk(&parent, version), nil
		case "Container":
			var err error
			var parent Java
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*Java).Container(&parent), nil
		default:
		...
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("Java").
					WithFunction(
						dag.NewFunction("WithMaven",
							dag.TypeDef().WithObject("Java")).
							WithArg("version", dag.TypeDef().WithKind(Stringkind))).
					WithFunction(
						dag.NewFunction("WithJdk",
							dag.TypeDef().WithObject("Java")).
							WithArg("version", dag.TypeDef().WithKind(Stringkind))).
					WithFunction(
						dag.NewFunction("Container",
							dag.TypeDef().WithObject("Container"))).
					WithField("Ctr", dag.TypeDef().WithObject("Container"))), nil
	default:
